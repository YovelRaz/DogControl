...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function createUpperPart(unit) {
        ...
        ///WE DECIDED TO REMOVE THE TITLE FROM HERE
    ...
    function createBottomPart(unit) {
        ...
        enterButton.textContent = 'כניסה לתצוגת לקוח';
    ...
    function createDropdown(unit) {
        ...
        const options = {
            ...
            'ניהול מדיה': () => window.location.href = `MediaPage.html?unit_id=${unit.unit_id}`,
            ...
            <div class="card">
                ...
                <div class="rm-bg card-footer text-muted d-flex flex-column align-items-center">
                    <p style="width: 100%; text-align: center;">תאריך עדכון אחרון: ${latestUpdateDate}</p>
                                    <button class="custom-enter-button" onclick="window.location.href='ViewPage.html?unitid=${unit.unit_id}&pageid=${unit.pages[0].page_id}'">כניסה לתצוגת לקוח</button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // If the user is not on the login page and is not logged in, redirect to the login page
    if (!isLoggedIn && !currentPath.includes('logIn.html')) {
        window.location.href = 'logIn.html';
        return; // Stop further script execution
    }

    // Handle login if the user is on the login page
